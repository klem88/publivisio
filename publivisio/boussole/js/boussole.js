...
function aggregatedfilter(searchlinkvalue){	
	...
	let nodesforfilterlists = nodesdetails.filter(function(d){
			return d.docid.length > 0 & 
				((!(institutionvalue == allinstitutionskeyword)) ? referenceIntitutions[d.iid].name == institutionvalue : true) &
				((!(countryvalue == allcountrieskeyword)) ? referenceCountries[referenceIntitutions[d.iid].countryid].name == countryvalue : true);
		});

	// EXTRACT THE UNIQUE DOCID FROM THE REMAINING NODES
	let tempdocid = [];
	nodesforfilterlists.map(function(d){ return d.docid.map(function(e){ tempdocid.push(e) }); })
	...
	//COUNTRIES
	let temp = nodesforfilterlists.map(function(d){
	...
	// INSTITUTIONS
	let tempinst = {};
	nodesforfilterlists.map(function(d){
